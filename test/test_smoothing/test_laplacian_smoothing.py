...
def create_noisy_sphere(n_points: int = 100, noise_scale: float = 0.1, seed: int = 0) -> Mesh:
...
def test_relaxation_factor_scaling():
    ...
    smoothed_small = smooth_laplacian(mesh, n_iter=1, relaxation_factor=0.01, inplace=False)
    displacement_small = torch.norm(smoothed_small.points - mesh.points, dim=-1).max()
    
    # Single iteration with large factor
    smoothed_large = smooth_laplacian(mesh, n_iter=1, relaxation_factor=0.1, inplace=False)
...
def test_n_iter_behavior():
    ...
    smoothed_10 = smooth_laplacian(mesh, n_iter=10, relaxation_factor=0.05, inplace=False)
    roughness_10 = measure_roughness(smoothed_10)
    
    smoothed_50 = smooth_laplacian(mesh, n_iter=50, relaxation_factor=0.05, inplace=False)
...
def test_inplace_vs_copy():
    ...
    smoothed_copy = smooth_laplacian(mesh, n_iter=10, relaxation_factor=0.05, inplace=False)
    ...
    # Test inplace=True
    smoothed_inplace = smooth_laplacian(mesh, n_iter=10, relaxation_factor=0.05, inplace=True)
    assert smoothed_inplace is mesh, \
        "inplace=True should return same object"
...
def test_boundary_fixed_when_enabled():
    ...
    assert torch.allclose(smoothed_boundary_points, original_boundary_points, atol=1e-6), \
        "Boundary vertices should not move when boundary_smoothing=True"
...
def test_boundary_moves_when_disabled():
    ...
    smoothed_boundary_points = smoothed.points[boundary_verts]
    max_displacement = torch.norm(smoothed_boundary_points - original_boundary_points, dim=-1).max()
...
def test_feature_detection_higher_codimension():
    ...
    points = torch.tensor([[0., 0., 0.], [1., 0., 0.], [2., 0., 0.]], dtype=torch.float32)
...
def test_feature_detection_no_sharp_edges():
    ...
    points = torch.tensor([
        [0., 0., 0.], [1., 0., 0.], [0.5, 0.866, 0.],  # Triangle 1
        [0., 0., 0.], [0.5, 0.866, 0.], [-0.5, 0.866, 0.],  # Triangle 2
    ], dtype=torch.float32)
    ...
    max_displacement = torch.norm(smoothed.points - original_points, dim=-1).max()
    assert max_displacement > 1e-6, \
        "Points should move when no sharp edges detected"
...
def test_feature_detection_no_interior_edges():
    ...
    points = torch.tensor([[0., 0., 0.], [1., 0., 0.], [0.5, 0.866, 0.]], dtype=torch.float32)
    ...
    max_displacement = torch.norm(smoothed.points - original_points, dim=-1).max()
    assert max_displacement > 1e-6, \
        "Points should move when no interior edges exist"
...
def test_convergence_early_exit():
    ...
    points = torch.tensor([
        [0., 0., 0.], [1., 0., 0.], [0.5, 0.866, 0.],  # First triangle
        [1., 0., 0.], [2., 0., 0.], [1.5, 0.866, 0.],  # Second triangle
    ], dtype=torch.float32)
    cells = torch.tensor([[0, 1, 2], [3, 4, 5]], dtype=torch.int64)
    mesh = Mesh(points=points, cells=cells)
    
    # Pre-smooth to make it nearly converged
    mesh = smooth_laplacian(mesh, n_iter=50, relaxation_factor=0.05, boundary_smoothing=False, inplace=True)
    ...
    bbox_diagonal = torch.norm(mesh.points.max(dim=0).values - mesh.points.min(dim=0).values)
...
def test_no_convergence_when_zero():
    ...
    smoothed_5 = smooth_laplacian(
        mesh, n_iter=5, relaxation_factor=0.1, convergence=0.0, boundary_smoothing=False, inplace=False
    )
    smoothed_10 = smooth_laplacian(
        mesh, n_iter=10, relaxation_factor=0.1, convergence=0.0, boundary_smoothing=False, inplace=False
    )
...
@pytest.mark.parametrize("n_spatial_dims,n_manifold_dims", [
    (2, 1),  # Curves in 2D
    (3, 1),  # Curves in 3D
    (3, 2),  # Surfaces in 3D
])
def test_dimensional_coverage(n_spatial_dims, n_manifold_dims):
    ...
    if n_manifold_dims == 1:
        # Line segments
        if n_spatial_dims == 2:
            points = torch.tensor([[0., 0.], [1., 0.], [1.5, 0.5], [2., 1.]], dtype=torch.float32)
            cells = torch.tensor([[0, 1], [1, 2], [2, 3]], dtype=torch.int64)
        else:  # 3D
            points = torch.tensor([[0., 0., 0.], [1., 0., 0.], [1.5, 0.5, 0.], [2., 1., 0.]], dtype=torch.float32)
            cells = torch.tensor([[0, 1], [1, 2], [2, 3]], dtype=torch.int64)
    else:  # n_manifold_dims == 2
        # Triangle in 3D
        points = torch.tensor([[0., 0., 0.], [1., 0., 0.], [0., 1., 0.], [1., 1., 0.]], dtype=torch.float32)
...
def test_single_triangle():
    ...
    points = torch.tensor([[0., 0., 0.], [1., 0., 0.], [0., 1., 0.]], dtype=torch.float32)
    cells = torch.tensor([[0, 1, 2]], dtype=torch.int64)
    mesh = Mesh(points=points, cells=cells)
...
def test_many_iterations():
    ...
    smoothed = smooth_laplacian(mesh, n_iter=1000, relaxation_factor=0.01, inplace=False)
...
def test_isolated_vertices():
    ...
    points = torch.tensor([
        [0., 0., 0.], [1., 0., 0.], [0., 1., 0.],  # Triangle vertices
...
def test_point_data_preserved():
    ...
    assert torch.allclose(smoothed.point_data["test_scalar"], mesh.point_data["test_scalar"])
    assert torch.allclose(smoothed.point_data["test_vector"], mesh.point_data["test_vector"])
...
def test_global_data_unchanged():
    ...
    assert torch.allclose(smoothed.global_data["test_value"], mesh.global_data["test_value"])
...
def test_device_compatibility(device):
    ...
    points = torch.tensor([[0., 0., 0.], [1., 0., 0.], [0., 1., 0.]], dtype=torch.float32, device=device)
