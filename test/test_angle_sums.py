...
class TestClosedSurfaceAngleSums:
    ...
    def create_sphere_mesh(self, radius=1.0, subdivisions=0, device="cpu"):
        ...
        for _ in range(subdivisions):
            ...
            mesh = Mesh(
                points=mesh.points / torch.norm(mesh.points, dim=-1, keepdim=True) * radius,
                cells=mesh.cells,
            )
...
class TestTriangleAngleSum:
    ...
    def test_triangle_angles_sum_to_pi(self, device):
        ...
        triangles = [
            # Equilateral
            torch.tensor([[0., 0., 0.], [1., 0., 0.], [0.5, math.sqrt(3)/2, 0.]], device=device),
            # Right triangle
            torch.tensor([[0., 0., 0.], [1., 0., 0.], [0., 1., 0.]], device=device),
            # Scalene
            torch.tensor([[0., 0., 0.], [2., 0., 0.], [0.5, 1.5, 0.]], device=device),
        ]
