...
class TestGradient:
    ...
    def test_quadratic_hessian_uniformity(self, tetbeam_mesh, method):
        ...
        assert cv < 0.5, f"Laplacian not uniform: CV={cv:.3f}, mean={mean_lap:.3f}, std={std_lap:.3f}"
...
class TestDivergence:
    ...
    def test_uniform_divergence_3d(self, tetbeam_mesh):
        ...
        expected = 3.0
        assert torch.allclose(divergence, torch.full_like(divergence, expected), atol=1e-4), \
...
class TestLaplacian:
    ...
    def test_dec_laplacian_linear_function_zero(self):
        ...
        assert torch.abs(lap[4]) < 1e-6, f"Laplacian of linear function at interior: {lap[4]:.6f}"
...
class TestCalculusIdentities:
    ...
    def test_curl_of_gradient_is_zero(self, tetbeam_mesh):
        ...
        mesh_grad_linear = mesh.compute_point_derivatives(keys="phi_linear", method="lsq")
        ...
        assert torch.allclose(curl_of_grad_linear, torch.zeros_like(curl_of_grad_linear), atol=1e-6)
...
class TestParametrized:
    ...
    def test_uniform_divergence_recovery(self, tetbeam_mesh, divergence_value):
        ...
        assert torch.allclose(div_v, torch.full_like(div_v, divergence_value), atol=1e-4)
