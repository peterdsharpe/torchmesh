...
def fill_holes(
    ...
    new_faces = torch.cat([
        boundary_edges,
        torch.full((n_boundary_edges, 1), centroid_idx, dtype=torch.long, device=device)
    ], dim=1)  # (n_boundary, 3)
    ...
    def extend_cell_data(tensor):
        ...
        if tensor.ndim == 1 or (tensor.ndim > 1 and tensor.shape[0] == mesh.n_cells):
            ...
            if tensor.ndim == 1:
                new_data = torch.full((n_boundary_edges,), fill_value, dtype=tensor.dtype, device=device)
            else:
                new_data = torch.full((n_boundary_edges, *tensor.shape[1:]), fill_value, dtype=tensor.dtype, device=device)
