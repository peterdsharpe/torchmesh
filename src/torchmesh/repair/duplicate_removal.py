...
def remove_duplicate_vertices(
    ...
    if n_original == 0:
        return mesh, {"n_duplicates_merged": 0, "n_points_original": 0, "n_points_final": 0}
    ...
    point_cells = torch.arange(n_original, device=device, dtype=torch.long).unsqueeze(1)  # (n_points, 1)
    ...
    old_to_new[unique_canonical] = torch.arange(n_unique, device=device, dtype=torch.long)
    ...
    new_point_data = TensorDict(point_data_filtered[unique_canonical], batch_size=[n_unique])
    new_cell_data = TensorDict(mesh.cell_data.exclude("_cache"), batch_size=mesh.cell_data.batch_size)
    new_global_data = TensorDict(mesh.global_data, batch_size=mesh.global_data.batch_size)
