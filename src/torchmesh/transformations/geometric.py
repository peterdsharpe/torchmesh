...
def _build_3d_rotation(u: torch.Tensor, angle: torch.Tensor | float, device) -> torch.Tensor:
...
def _build_rotation_matrix(
    ...
    if n_spatial_dims == 2:
        ...
        if axis.shape != (3,):
            raise ValueError(f"For 3D rotation, axis must have shape (3,), got {axis.shape}")
...
def _transform_higher_order_tensor(
    ...
    if n_dims == 2:
        ...
        # We'll apply matrix transformations sequentially for simplicity
        result = tensor
        for dim_idx in range(n_dims):
            # Contract dimension dim_idx+1 (accounting for batch dimension at 0)
            # Build einsum: matrix contracts with result along one dimension at a time
            result = torch.einsum(
                f'ij,b{"".join(["k" if i == dim_idx else chr(97+i) for i in range(n_dims)])}->b{"".join([chr(97+i) if i != dim_idx else "j" for i in range(n_dims)])}',
                matrix,
                result
            )
...
def _handle_caches_for_uniform_scale(
    ...
    if "_areas" in new_cell_data:
        new_cell_data["_areas"] = mesh.cell_data["_areas"] * (abs(factor) ** mesh.n_manifold_dims)
...
def rotate(
    ...
    if center is not None:
        center = torch.as_tensor(center, device=mesh.points.device, dtype=mesh.points.dtype)
        # Translate to origin
        mesh_centered = translate(mesh, -center)
        # Rotate
        mesh_rotated = rotate(mesh_centered, axis, angle, center=None, transform_data=transform_data)
        # Translate back
...
def scale(
    ...
    if isinstance(factor, (int, float)):
        ...
        scale_matrix = torch.eye(mesh.n_spatial_dims, device=mesh.points.device) * factor_scalar
    else:
        factor_tensor = torch.as_tensor(factor, device=mesh.points.device, dtype=mesh.points.dtype)
    ...
    if center is not None:
        center = torch.as_tensor(center, device=mesh.points.device, dtype=mesh.points.dtype)
        mesh_centered = translate(mesh, -center)
        mesh_scaled = scale(mesh_centered, factor, center=None, transform_data=transform_data)
