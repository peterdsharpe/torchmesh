...
def validate_mesh(
    ...
    if check_out_of_bounds:
        if mesh.n_cells > 0:
            ...
            if n_out_of_bounds > 0:
                results["valid"] = False
                results["out_of_bounds_cell_indices"] = torch.where(out_of_bounds_cells)[0]
    ...
    if check_duplicate_vertices:
        ...
        if mesh.n_points < 10000:  # Exact check for small meshes
            ...
            if n_duplicates > 0:
                results["valid"] = False
                results["duplicate_vertex_pairs"] = torch.stack(duplicate_indices, dim=1)
...
def check_duplicate_cell_vertices(mesh: "Mesh") -> tuple[int, torch.Tensor]:
    ...
    invalid_indices = torch.tensor(invalid_cells, dtype=torch.long, device=mesh.cells.device)
