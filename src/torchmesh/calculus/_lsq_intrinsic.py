...
def compute_point_gradient_lsq_intrinsic(
    ...
    gradients = torch.zeros(gradient_shape, dtype=point_values.dtype, device=mesh.points.device)
    ...
    if mesh.codimension == 1:
        ...
        for point_idx in range(n_points):
            ...
            if normal[0].abs() < 0.9:
                v1 = torch.tensor([1.0, 0.0, 0.0], device=normal.device, dtype=normal.dtype)
            else:
                v1 = torch.tensor([0.0, 1.0, 0.0], device=normal.device, dtype=normal.dtype)
            ...
            if is_scalar:
                ...
                try:
                    ...
                    # Convert to ambient
                    grad_ambient = tangent_basis @ grad_tangent  # (n_spatial_dims, n_components)
                    grad_ambient_reshaped = grad_ambient.reshape(n_spatial_dims, *orig_shape)
