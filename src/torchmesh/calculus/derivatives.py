...
def _get_field_value(data_dict: TensorDict, key_path: str | tuple[str, ...]) -> torch.Tensor:
    ...
    for key_path, field_name in key_list:
        ...
        if method == "lsq":
            if gradient_type == "intrinsic":
                ...
                grad_intrinsic = compute_gradient_points_lsq(mesh, field_values, intrinsic=True)
                grad_extrinsic = None  # Not computed
            elif gradient_type == "extrinsic":
                # Standard LSQ in ambient space
                grad_extrinsic = compute_gradient_points_lsq(mesh, field_values, intrinsic=False)
...
def compute_cell_derivatives(
    ...
    for key_path, field_name in key_list:
        ...
        if method == "lsq":
            ...
            raise NotImplementedError("DEC cell gradients not yet implemented. Use method='lsq'.")
        else:
        ...
        if gradient_type == "extrinsic":
            ...
            output_path = (key_path[:-1] + (output_key,)) if isinstance(key_path, tuple) else output_key
            ...
            output_path_ext = (key_path[:-1] + (output_key_ext,)) if isinstance(key_path, tuple) else output_key_ext
            output_path_int = (key_path[:-1] + (output_key_int,)) if isinstance(key_path, tuple) else output_key_int
