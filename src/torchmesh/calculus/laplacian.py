...
def compute_laplacian_points_dec(
    ...
    if point_values.ndim == 1:
        laplacian = torch.zeros(n_points, dtype=point_values.dtype, device=mesh.points.device)
    else:
    ...
    if point_values.ndim == 1:
        ...
        laplacian = laplacian / dual_volumes_0.view(-1, *([1]*(point_values.ndim-1))).clamp(min=1e-10)
