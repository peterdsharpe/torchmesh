...
def estimate_tangent_space_pca(
    ...
    effective_counts = torch.minimum(
        neighbor_counts, 
        torch.tensor(k_neighbors, dtype=torch.int64, device=device)
    )
    ...
    for count_idx, n_neighbors in enumerate(unique_counts):
        ...
        tangent_vecs = eigenvectors_sorted[:, :, :n_manifold_dims]  # (n_group, n_spatial_dims, n_manifold_dims)
        tangent_basis[point_indices] = tangent_vecs.transpose(1, 2)  # (n_group, n_manifold_dims, n_spatial_dims)
        
        # Remaining eigenvectors span normal space
        normal_vecs = eigenvectors_sorted[:, :, n_manifold_dims:]  # (n_group, n_spatial_dims, codimension)
        normal_basis[point_indices] = normal_vecs.transpose(1, 2)  # (n_group, codimension, n_spatial_dims)
