...
def _text_to_path(
    ...
    while i < len(codes):
        ...
        if code == Path.MOVETO:
            ...
            sampled = _sample_curve_segment(path_points[-1], [verts[i]], verts[i + 1], num_samples)
            path_points.extend(sampled[1:])
            i += 2
        elif code == Path.CURVE4:
            dist = torch.norm(verts[i + 2] - path_points[-1]).item()
            num_samples = max(5, int(dist * samples_per_unit))
            sampled = _sample_curve_segment(path_points[-1], [verts[i], verts[i + 1]], verts[i + 2], num_samples)
            path_points.extend(sampled[1:])
            i += 3
    ...
    for edge in edges:
        ...
        if edge_length <= max_length:
            ...
            for j in range(1, n_segments):
                ...
                refined_edges.append(torch.tensor([prev_idx, next_idx], dtype=torch.int64))
                prev_idx = next_idx
                next_idx += 1
...
def _group_letters(text_path: Path):
    ...
    for outer_start, outer_end in outers:
        ...
        letter_groups.append({'outer': (outer_start, outer_end), 'holes': contained_holes})
...
def _winding_number(points: torch.Tensor, path: Path) -> torch.Tensor:
    ...
    for i, start_idx in enumerate(moveto_indices):
        end_idx = int(moveto_indices[i + 1]) if i < len(moveto_indices) - 1 else len(path_codes)
...
def _triangulate(points, edges, text_path):
    ...
    for group in letter_groups:
        ...
        letter_point_indices = _get_letter_points(points, edges, text_path, all_polygon_ranges)
...
def text_3d_3d(
    ...
    # Create 2D mesh
    mesh_2d = text_2d_2d(text, font_size, samples_per_unit, max_segment_length, device="cpu")
    
    # Embed to 3D and extrude
    mesh_3d_surface = embed_in_spatial_dims(mesh_2d, target_n_spatial_dims=3)
    volume = extrude(
        mesh_3d_surface,
        vector=torch.tensor([0.0, 0.0, extrusion_height], device=mesh_3d_surface.points.device),
    )
